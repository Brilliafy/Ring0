...
fn snake_to_camel(s: &str) -> String {
    ...
    for c in s.chars() {
        if c == '_' { upper_next = true; }
        else if upper_next { result.push(c.to_ascii_uppercase()); upper_next = false; }
        else { result.push(c); }
    }
    result
}
...
fn main() {
    ...
    if let Some(src) = generated {
        ...
        for line in content.lines() {
            ...
            if (l.contains("pub fn set_") || l.contains("pub fn get_") || l.contains("pub fn init_")) && !l.contains("field_type") && !l.contains("annotation_type") {
...
fn find_file(dir: &Path, target: &str) -> Option<PathBuf> {
    if let Ok(entries) = std::fs::read_dir(dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                if let Some(found) = find_file(&path, target) { return Some(found); }
