...
const SIGNATURES: &[Signature] = &[
    ...
    Signature { rule_id: 5003, name: "ET WEB_SERVER SQL injection SELECT..FROM", pattern: "(?i)select.{0,200}from", severity: 3 },
    ...
    Signature { rule_id: 5010, name: "ET POLICY powershell encoded command", pattern: "(?i)powershell.{0,50}-e(nc|ncod)", severity: 3 },
    Signature { rule_id: 5011, name: "ET TROJAN suspicious wget -O", pattern: "(?i)wget.{0,40}-O", severity: 2 },
    Signature { rule_id: 5012, name: "ET TROJAN suspicious curl -o", pattern: "(?i)curl.{0,40}-o", severity: 2 },
    Signature { rule_id: 5013, name: "ET TROJAN Cobalt Strike beacon", pattern: "(?i)MZ.{0,2000}(?i)beacon", severity: 4 },
    Signature { rule_id: 5014, name: "ET MALWARE webshell one-liner", pattern: "(?i)(assert|system|exec)\\(\\$_", severity: 4 },
    Signature { rule_id: 5015, name: "ET WEB_SERVER PHP code injection", pattern: "\\$_GET\\[.{0,64}\\].{0,32}(exec|system|shell_exec)", severity: 4 },
    ...
    Signature { rule_id: 5019, name: "ET TROJAN C2 check-in", pattern: "(?i)(POST|GET).{0,100}(/api/|/gate|/c2/|/panel)", severity: 2 },
